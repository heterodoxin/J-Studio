...
from jlens.interventions import InterventionTrace, SearchPoint
from jlens.vis import (
    SliceData,
    build_page,
    write_intervention_trace,
    write_slice_files,
)
...
def _trace() -> InterventionTrace:
    return InterventionTrace(
        ...
        selected_scale=0.625,
        normalized_cost=0.42,
        baseline_scores={"3": 2.0, "7": -1.0},
        after_scores={"3": 0.2, "7": 0.3},
        baseline_top_ids=(3, 1, 2),
        after_top_ids=(7, 1, 3),
        search_points=(SearchPoint(0.625, True, 0.1, 0.2),),
        warnings=("uncalibrated identity geometry",),
    )
...
def test_page_uses_jstudio_graphite_theme_tokens():
    page, _, _ = build_page(
        _synthetic_slice(), "p", title="T", description="d", mode="embed"
    )
...
def test_write_intervention_trace(tmp_path):
    trace = _trace()

    path = write_intervention_trace(trace, tmp_path)

    assert path.name == "intervention.json"
    assert json.loads(path.read_text())["operation"] == "replace"
    assert not (tmp_path / "intervention.json.tmp").exists()


def test_embed_page_contains_intervention_summary():
    page, _, _ = build_page(
        _synthetic_slice(),
        "p",
        title="T",
        description="d",
        intervention_trace=_trace(),
    )

    assert '"intervention"' in page
    assert '"selected_scale": 0.625' in page
    assert "replace 3 → 7" in page


def test_fetch_mode_writes_intervention_sidecar(tmp_path):
    build_page(
        _synthetic_slice(),
        "p",
        title="T",
