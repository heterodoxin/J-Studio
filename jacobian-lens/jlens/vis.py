...
from jlens.interventions import InterventionTrace
...
def compute_slice(
    ...
    *,
    ...
    def lens_logits(layer: int) -> torch.Tensor:
        ...
        if layer in lens.source_layers:
...
def write_intervention_trace(trace: InterventionTrace, out_dir: str | Path) -> Path:
    """Atomically write a UI-neutral intervention trace sidecar."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    destination = out_dir / "intervention.json"
    temporary = out_dir / "intervention.json.tmp"
...
def _intervention_summary(trace: InterventionTrace | None) -> str:
    if trace is None:
        return ""
    target = ", ".join(str(token_id) for token_id in trace.target_ids)
    source = ", ".join(str(token_id) for token_id in trace.source_ids)
    concept = f"{source} → {target}" if source else target
    warnings = " · ".join(trace.warnings)
    warning_html = (
        f'<span class="intervention-warning">{html.escape(warnings)}</span>'
        if warnings
        else ""
    )
    return (
        '<div class="intervention-summary">'
        f"<b>{html.escape(trace.operation)} {html.escape(concept)}</b>"
        f"<span>L{trace.selected_layer} P{list(trace.selected_positions)}</span>"
        f"<span>scale={trace.selected_scale:.4g}</span>"
        f"<span>cost={trace.normalized_cost:.4g}</span>"
        f"{warning_html}</div>"
    )
...
def build_page(
    ...
    *,
    ...
    alt_token: dict[int, str] | None = None,
    intervention_trace: InterventionTrace | None = None,
) -> tuple[str, int, int]:
    ...
    if mode == "embed":
        ...
        if intervention_trace is not None:
            bootstrap["intervention"] = intervention_trace.to_dict()
        ...
        if intervention_trace is not None:
            write_intervention_trace(intervention_trace, out_dir)
            bootstrap["intervention"] = intervention_trace.to_dict()
    ...
    page = (
        ...
        .replace("__INTERVENTION__", _intervention_summary(intervention_trace))
